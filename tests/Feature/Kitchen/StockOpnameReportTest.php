...
/**
 * Domain F item C: sesi stok opname multi-barang dan layar gabungan
 * riwayat/sirkulasi. Paralel persis dengan
 * tests/Feature/Inventory/StockOpnameReportTest.php (domain E item C).
 */
class StockOpnameReportTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->petugas = User::query()->create([
            'username' => 'uji-opname-dapur', 'name' => 'Petugas Opname Dapur Uji', 'password' => 'password', 'is_active' => true,
        ]);
        ...
        $this->barangB = $this->items->createItem([
            'code' => 'DPR002', 'name' => 'Minyak Goreng', 'category_id' => $kategori->id,
            'unit_of_measure' => 'liter', 'quantity_on_hand' => 0, 'is_active' => true,
        ]);
    ...
    public function opname_selesai_menyesuaikan_selisih_lewat_ledger(): void
    {
        ...
        $this->opnames->recordCount($opname, $barisA->id, 95, 'Susut 5 kg');
    ...
    #[Test]
    public function layar_opname_dan_laporan_hanya_untuk_petugas_dapur(): void
    {
        $this->actingAs($this->petugas)->get(route('kitchen.opname.index'))->assertOk();
        $this->actingAs($this->petugas)->get(route('kitchen.laporan.index'))->assertOk();

        $dokter = User::query()->create([
            'username' => 'uji-dokter-opname-dapur', 'name' => 'Dokter Uji', 'password' => 'password', 'is_active' => true,
        ]);
        $dokter->roles()->attach(Role::query()->where('code', 'dokter')->firstOrFail());

        $this->actingAs($dokter)->get(route('kitchen.opname.index'))->assertForbidden();
        $this->actingAs($dokter)->get(route('kitchen.laporan.index'))->assertForbidden();
    }
    ...
    public function sesi_opname_bisa_dibuka_dan_diselesaikan_lewat_http(): void
    {
        ...
        $this->actingAs($this->petugas)->post(route('kitchen.opname.hitung', $opname), [
            'baris_id' => [$barisA->id],
            'counted_quantity' => [90],
        ])->assertRedirect();

        $this->actingAs($this->petugas)->post(route('kitchen.opname.selesai', $opname))->assertRedirect();
