...
/**
 * pengajuan_barang_dapur (+ permintaan_dapur, listing sama): diajukan
 * -> disetujui/ditolak -> (kalau disetujui) selesai setelah stok
 * benar-benar dikeluarkan. Pola identik dengan
 * Inventory\Services\RequisitionService.
 */
class RequisitionService
{
    ...
    {
        if ($items === []) {
            throw new KitchenException('Permintaan harus berisi minimal satu barang.');
        }
    ...
    public function approve(Requisition $requisition, int $decidedBy): Requisition
    {
        if (! $requisition->isPending()) {
            throw new KitchenException('Permintaan ini sudah diputuskan sebelumnya.');
        }
    ...
    public function fulfill(Requisition $requisition, User $actor): Requisition
    {
        if ($requisition->status !== Requisition::STATUS_DISETUJUI) {
            throw new KitchenException('Permintaan harus disetujui lebih dulu sebelum barangnya dikeluarkan.');
        }
