...
/**
 * dapur_opname — sesi hitung fisik multi-barang, beda dari
 * MasterDataController::opname() yang ad-hoc 1 barang (tetap ada, di
 * bawah dapur_barang, untuk koreksi cepat di luar sesi resmi). Selisih
 * disesuaikan lewat StockLedger::opname() per baris saat sesi
 * diselesaikan — satu-satunya jalan penyesuaian, baik dari sini
 * maupun dari opname() ad-hoc.
 */
class StockOpnameService
{
    ...
    public function recordCount(StockOpname $opname, int $baris, float $counted, ?string $note = null): void
    {
        if ($opname->status !== StockOpname::STATUS_DRAF) {
            throw new KitchenException('Opname ini sudah diselesaikan.');
        }
