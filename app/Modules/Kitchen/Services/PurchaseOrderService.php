...
/**
 * dapur_pembelian — PO ke suplier dapur, boleh berasal dari pengajuan
 * unit atau berdiri sendiri. surat_pemesanan_dapur dilayani lewat
...
class PurchaseOrderService
{
    ...
    {
        if ($items === []) {
            throw new KitchenException('PO harus berisi minimal satu barang.');
        }
    ...
    public function submit(PurchaseOrder $po): PurchaseOrder
    {
        if ($po->status !== PurchaseOrder::STATUS_DRAF) {
            throw new KitchenException('Hanya PO berstatus draf yang bisa dikirim ke suplier.');
        }
    ...
    public function cancel(PurchaseOrder $po): PurchaseOrder
    {
        if (in_array($po->status, [PurchaseOrder::STATUS_DITERIMA, PurchaseOrder::STATUS_DIBATALKAN], true)) {
            throw new KitchenException('PO yang sudah diterima penuh atau sudah dibatalkan tidak bisa dibatalkan lagi.');
        }
    ...
    /** Dipanggil GoodsReceiptService setelah barang diterima — load() paksa, bukan loadMissing(), lihat catatan yang sama di inventory\PurchaseOrderService/pharmacy\PurchaseOrderService. */
