...
@section('title', 'Dapur — Pengadaan')
@section('breadcrumb', 'Konteks kitchen')
@section('heading', 'Pengadaan & Pemesanan Barang Dapur')

@section('actions')
  <a href="{{ route('kitchen.permintaan.index') }}" class="btn btn-link">&larr; Permintaan</a>
  @can('dapur_pemesanan')
    <a href="{{ route('kitchen.penerimaan.index') }}" class="btn btn-outline-primary">Penerimaan</a>
  @endcan
  @can('dapur_returbeli')
...
<div class="card mb-3">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('kitchen.po.simpan') }}">
      @csrf
      ...
      <tbody>
        @forelse ($po as $p)
          <tr>
            ...
            <td><a href="{{ route('kitchen.po.show', $p) }}" class="btn btn-sm btn-outline-primary">Detail</a></td>
          </tr>
        @empty
