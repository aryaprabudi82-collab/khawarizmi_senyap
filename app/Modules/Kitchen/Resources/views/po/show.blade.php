...
@section('actions')
  <a href="{{ route('kitchen.po.index') }}" class="btn btn-link">&larr; Daftar PO</a>
  @can('surat_pemesanan_dapur')
    <a href="{{ route('kitchen.po.cetak', $po) }}" class="btn btn-outline-secondary" target="_blank">Cetak Surat Pemesanan</a>
  @endcan
@endsection
  ...
  <div class="col-12 col-lg-7">
    <div class="card mb-3">
      ...
      <div class="card-body border-top d-flex gap-2">
        @if ($po->status === 'draf')
          <form method="POST" action="{{ route('kitchen.po.kirim', $po) }}">
            @csrf
            <button class="btn btn-sm btn-outline-primary">Kirim ke Suplier</button>
          </form>
        @endif
        @if (! in_array($po->status, ['diterima', 'dibatalkan']))
          <form method="POST" action="{{ route('kitchen.po.batal', $po) }}" onsubmit="return confirm('Batalkan PO ini?')">
            @csrf
    ...
    @can('dapur_pemesanan')
      @if (in_array($po->status, ['dipesan', 'diterima-sebagian']))
        <div class="card mb-3">
          ...
          <div class="card-body">
            <form method="POST" action="{{ route('kitchen.penerimaan.simpan', $po) }}">
              @csrf
