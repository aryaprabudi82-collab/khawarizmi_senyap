...
@section('actions')
  <a href="{{ route('kitchen.opname.index') }}" class="btn btn-link">&larr; Daftar Opname</a>
@endsection
...
<div class="card">
  ...
  <div class="card-body">
    @if ($opname->status === 'draf')
      <form method="POST" action="{{ route('kitchen.opname.hitung', $opname) }}" class="mb-3">
        @csrf
      ...
      <form method="POST" action="{{ route('kitchen.opname.selesai', $opname) }}" onsubmit="return confirm('Selesaikan opname? Selisih akan disesuaikan sebagai koreksi stok.')">
        @csrf
