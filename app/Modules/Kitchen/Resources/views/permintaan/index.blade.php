...
@section('title', 'Dapur — Permintaan Barang')
@section('breadcrumb', 'Konteks kitchen')
@section('heading', 'Permintaan Barang Dapur dari Unit')
...
<div class="card mb-3">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('kitchen.permintaan.simpan') }}">
      @csrf
          ...
          <tr>
            ...
            <td>
              @if ($p->status === 'diajukan')
                <div class="btn-group">
                  <form method="POST" action="{{ route('kitchen.permintaan.setuju', $p) }}">
                    @csrf
              ...
              @elseif ($p->status === 'disetujui')
                <form method="POST" action="{{ route('kitchen.permintaan.keluarkan', $p) }}">
                  @csrf
  ...
  @if ($p->status === 'diajukan')
    <div class="modal fade" id="tolak-{{ $p->id }}" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="POST" action="{{ route('kitchen.permintaan.tolak', $p) }}">
          @csrf
