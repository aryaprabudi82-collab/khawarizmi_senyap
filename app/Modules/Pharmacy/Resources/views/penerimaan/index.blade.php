...
@section('title', 'Farmasi — Penerimaan Obat & BHP')
@section('breadcrumb', 'Konteks pharmacy')
@section('heading', 'Penerimaan & Pembayaran Obat, Alkes & BHP')

@section('actions')
  <a href="{{ route('pharmacy.po.index') }}" class="btn btn-link">&larr; PO</a>
@endsection
...
<div class="card">
  ...
  <div class="table-responsive">
    <table class="table table-vcenter card-table">
      <thead><tr><th>No. Terima</th><th>PO</th><th>Suplier</th><th>Tanggal</th><th>Pembayaran</th><th>Status</th><th class="w-1"></th></tr></thead>
      <tbody>
        @forelse ($penerimaan as $r)
          <tr>
            ...
            <td>
              <span class="badge bg-{{ $r->payment_status === 'lunas' ? 'green' : 'orange' }}-lt">{{ $r->payment_status }}</span>
              @if ($r->invoice_number)
                <div class="text-secondary small">{{ $r->invoice_number }}</div>
              @endif
            </td>
            <td><span class="badge bg-{{ $r->status === 'terverifikasi' ? 'green' : 'yellow' }}-lt">{{ $r->status }}</span></td>
            <td>
              @can('verifikasi_penerimaan_farmasi')
        ...
        @empty
          <tr><td colspan="7" class="text-center text-secondary py-3">Belum ada penerimaan.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
  ...
  @if ($r->status !== 'terverifikasi')
    <div class="modal fade" id="verifikasi-{{ $r->id }}" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="POST" action="{{ route('pharmacy.penerimaan.verifikasi', $r) }}">
          @csrf
