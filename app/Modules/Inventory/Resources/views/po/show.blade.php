...
@section('actions')
  <a href="{{ route('inventory.po.index') }}" class="btn btn-link">&larr; Daftar PO</a>
  @can('surat_pemesanan_non_medis')
    <a href="{{ route('inventory.po.cetak', $po) }}" class="btn btn-outline-secondary" target="_blank">Cetak Surat Pemesanan</a>
  @endcan
@endsection
    ...
    <div class="card mb-3">
      ...
      <div class="table-responsive">
        <table class="table table-vcenter card-table">
          <thead><tr><th>Barang</th><th>Satuan</th><th class="text-end">Dipesan</th><th class="text-end">Diterima</th><th class="text-end">Harga</th></tr></thead>
          <tbody>
            @foreach ($po->items as $baris)
              <tr>
                <td>{{ $baris->item_name }}</td>
                <td class="text-secondary small">{{ $baris->unit_of_measure }}</td>
      ...
      <div class="card-body border-top d-flex gap-2">
        @if ($po->status === 'draf')
          <form method="POST" action="{{ route('inventory.po.kirim', $po) }}">
            @csrf
            <button class="btn btn-sm btn-outline-primary">Kirim ke Suplier</button>
          </form>
        @endif
        @if (! in_array($po->status, ['diterima', 'dibatalkan']))
          <form method="POST" action="{{ route('inventory.po.batal', $po) }}" onsubmit="return confirm('Batalkan PO ini?')">
            @csrf
    ...
    @can('penerimaan_non_medis')
      @if (in_array($po->status, ['dipesan', 'diterima-sebagian']))
        <div class="card mb-3">
          ...
          <div class="card-body">
            <form method="POST" action="{{ route('inventory.penerimaan.simpan', $po) }}">
              @csrf
                    ...
                    @if ($baris->remainingQuantity() > 0)
                      <tr>
                        <td class="small">
                          ...
                          <div class="text-secondary">sisa {{ rtrim(rtrim(number_format($baris->remainingQuantity(), 2, ',', '.'), '0'), ',') }} {{ $baris->unit_of_measure }}</div>
                        </td>
