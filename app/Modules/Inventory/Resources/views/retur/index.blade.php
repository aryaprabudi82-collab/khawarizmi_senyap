...
@section('title', 'Logistik — Retur Ke Suplier')
@section('breadcrumb', 'Konteks inventory')
@section('heading', 'Retur Barang Non-Medis Ke Suplier')
...
<div class="card mb-3">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('inventory.retur.simpan') }}">
      @csrf
          ...
          <tr>
            ...
            <td>
              @if ($r->status === 'diajukan')
                <form method="POST" action="{{ route('inventory.retur.selesai', $r) }}" onsubmit="return confirm('Selesaikan retur ini? Stok terkait akan dikurangi.')">
                  @csrf
