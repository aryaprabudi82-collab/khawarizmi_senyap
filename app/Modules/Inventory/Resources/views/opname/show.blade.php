...
@section('heading', 'Opname ' . $opname->opname_number)

@section('actions')
  <a href="{{ route('inventory.opname.index') }}" class="btn btn-link">&larr; Daftar Opname</a>
@endsection
...
<div class="card">
  ...
  <div class="card-body">
    @if ($opname->status === 'draf')
      <form method="POST" action="{{ route('inventory.opname.hitung', $opname) }}" class="mb-3">
        @csrf
        <div class="table-responsive mb-2">
          <table class="table table-sm">
            <thead><tr><th>Barang</th><th class="text-end">Sistem</th><th style="width:130px">Hasil Hitung</th></tr></thead>
            <tbody>
              @foreach ($opname->items as $baris)
                <tr>
                  <td>
                    {{ $baris->item->name }}
                    <input type="hidden" name="baris_id[]" value="{{ $baris->id }}">
                  </td>
      ...
      <form method="POST" action="{{ route('inventory.opname.selesai', $opname) }}" onsubmit="return confirm('Selesaikan opname? Selisih akan disesuaikan sebagai koreksi stok.')">
        @csrf
    ...
    @else
      <div class="table-responsive">
        <table class="table table-vcenter card-table">
          <thead><tr><th>Barang</th><th class="text-end">Sistem</th><th class="text-end">Hitung Fisik</th><th class="text-end">Selisih</th></tr></thead>
          <tbody>
            @foreach ($opname->items as $baris)
              <tr>
                <td>{{ $baris->item->name }}</td>
                <td class="text-end font-monospace">{{ rtrim(rtrim(number_format((float) $baris->system_quantity, 2, ',', '.'), '0'), ',') }}</td>
