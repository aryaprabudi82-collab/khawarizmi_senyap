...
/**
 * ipsrs_returbeli — mengembalikan barang ke suplier. Lewat
 * StockLedger::issue() (source='retur-suplier', sudah terdaftar sejak
 * migrasi awal tapi belum pernah benar-benar dipanggil kode manapun).
 */
class SupplierReturnService
{
    ...
    {
        if ($items === []) {
            throw new InventoryException('Retur harus berisi minimal satu barang.');
        }
    ...
    public function complete(SupplierReturn $retur, User $actor): SupplierReturn
    {
        if ($retur->status !== SupplierReturn::STATUS_DIAJUKAN) {
            throw new InventoryException('Retur ini sudah selesai diproses.');
        }
